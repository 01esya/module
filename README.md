...
Информационная система формирования электронных путевых листов (ЭПЛ) с модулем имитационного спутникового GPS/ГЛОНАСС-мониторинга транспортных средств и ИИ-аудитом рейсов.

Система предназначена для автоматизации документооборота автотранспортного предприятия: выписка путевых листов, управление штатом водителей, визуализация геопозиции и состояния транспортных средств на основе имитационных навигационных данных, аналитика рейсов с помощью ИИ.

## 🎯 Основные функции системы

* 📄 **Электронные путевые листы (ЭПЛ):** выписка и ведение путевых листов по форме Минтранса РФ № 4-П. Поддержка валидации, смены статусов рейса и генерация печатных версий документов в формат PDF.
* 🗺️ **Имитационный GPS/ГЛОНАСС-мониторинг:** визуализация местоположения транспортных средств на интерактивной ГИС-карте (OpenLayers) на основе данных от программного симулятора телеметрии ТС с автоматическим построением маршрутов по алгоритму OSRM.
* 📈 **Имитационная телеметрия:** генерация навигационных пакетов (скорость, расход топлива, количество спутников) встроенным программным симулятором с шагом обновления данных в 12 секунд.
* 🧠 **ИИ-аудит и LLM-аналитика:** автоматический анализ безопасности рейса, оценка рисков перерасхода топлива, утомляемости водителя и исправности ТС через API OpenRouter.
* 👥 **Картотека сотрудников (Штат):** ведение базы водителей и диспетчеров с ролевой валидацией полей и водительских удостоверений.
* 🗄️ **Отказоустойчивая архитектура (MDM):** использование централизованных справочников в Supabase (PostgreSQL) с прозрачным автоматическим переключением на локальный SQLite-кэш при потере связи.
...
Cтатистика по рейсам, автопарку и предупреждения телеметрии.
![Панель управления](docs/screenshot_dashboard.png)

### 3. Интерактивная ГИС-карта
Отображение текущей геопозиции транспортных средств на карте с автоматическим расчетом и построением маршрутов рейсов.
...
### 6. Картотека сотрудников (Динамическая форма)
Интерфейс создания учетной записи сотрудника динамически изменяет набор полей в зависимости от выбранной роли (паттерн реактивных форм):
* **При выборе роли «Водитель»** автоматически отображаются поля для ввода номера водительского удостоверения и выбора категорий прав (A, B, C, D, E):
  ![Картотека сотрудников - роль Водитель](docs/screenshot_personnel.png)
* **При выборе роли «Диспетчер»** поля водительского удостоверения автоматически скрываются, так как они не требуются для данной должности:
  ![Картотека сотрудников - роль Диспетчер](docs/screenshot_personnel_dispatcher.png)

### 7. Печатная форма путевого листа (PDF)
Создание печатного документа по межотраслевой типовой форме № 4-П.
![Печатная форма путевого листа](docs/screenshot_print.png)
