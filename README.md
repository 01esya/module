...
Система предназначена для автоматизации документооборота автотранспортного предприятия: выписка путевых листов, управление штатом водителей, визуализация геопозиции и состояния транспортных средств на основе навигационных данных, аналитика рейсов с помощью ИИ.
...
* **Имитационный GPS/ГЛОНАСС-мониторинг:** визуализация местоположения транспортных средств на интерактивной ГИС-карте (OpenLayers) на основе данных от программного симулятора телеметрии ТС с автоматическим построением маршрутов по алгоритму OSRM.
* **Телеметрия:** генерация навигационных пакетов (скорость, расход топлива, количество спутников) встроенным программным симулятором с шагом обновления данных в 12 секунд.
...
* **Отказоустойчивая архитектура (MDM):** использование централизованных справочников в Supabase (PostgreSQL) с прозрачным автоматическим переключением на локальный SQLite-кэш при потере связи.
